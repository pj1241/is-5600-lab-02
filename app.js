document.addEventListener('DOMContentLoaded', () => {
  ...
  saveButton.addEventListener('click', (event) => {
    ...
    if (existingUserIndex !== -1) {
      ...
      // Add a new user
      const newUser = {
        id: id || Date.now(), // Generate a new ID if not provided
        user: { firstname, lastname, address, city, email },
        portfolio: [] // Initialize with an empty portfolio
      };
      userData.push(newUser);
    }

    generateUserList(userData, stocksData);
    document.querySelector('.userEntry').reset(); // Clear form after saving
  });
  ...
  function viewStock(symbol, stocks) {
    ...
    if (stockArea) {
      ...
      if (stock) {
        ...
        document.querySelector('#logo').src = `logos/${symbol}.svg`; // Make sure 'logos' folder is correct
      }
    }
  }
});
